...
unsigned char uart_rx[4];       //buffer for received bytes
unsigned short PosAltX, PosAltY, PosAltZ, PosAltT;  //previous positions
unsigned short PosAltI,PosI=3000,PosImax=4095;   //Laser-Intensity
...
unsigned long focus(void)    //Auto focus function implemented by pavan
{   unsigned long  SysTickScanStep;
...
int main(void)
{
    ...
    if((mode == MODE_MULTINSSCAN) && (SysTick >= SysTickScanStep))     //Multi Intensity Scan Modus implemented by pavan

    {
        ...
        if((voltage<vol_limit)&&(flage_rev==1)){     // NOT A BINARY SEARCH METHOD
        ...
        if((voltage<vol_limit)) //||(PosI > PosImax)  // Here we check the latchup condition and also the Intensity range loop

        //voltage<100,( SEE_FLAG>1  add SEE_FLAG=0),current>13, 0x08 & P1IN
        {
            flage_rev=1;
            PosI=PosImin;
            MCP4728_MultiWrite(0, PosI);
...
send:
        if(flage_rev==0){
            ...
            UART_TX((unsigned char)(PosI >> 8));
            UART_TX((unsigned char)PosI);

            UART_TX((unsigned char)(current >> 8));
            UART_TX((unsigned char)current);

            UART_TX((unsigned char)(voltage >> 8));
            UART_TX((unsigned char)voltage);
